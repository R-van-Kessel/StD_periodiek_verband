...
function generateQuestions() {
    ...
    // ── Keuzemogelijkheden voor random generatie ──
    // Amplitude: kleine gehele getallen
    let amplitudes  = [1, 2, 3, 4, 5];
    // Periode: nette waarden zodat tikjes op halve periode kloppen
    let periodes    = [2, 4, 6, 8, 10, 12];
    // Vraagtypes
    let types       = ['amplitude', 'periode', 'evenwicht'];

    // Genereer één willekeurige sinusgolf
    // Regel: m >= a zodat golf boven t-as blijft
    // Regel: m + a <= 10 zodat labels niet te groot worden
    function maakGolf() {
        let a = random(amplitudes);
        // evenwicht: minimaal a (zodat min >= 0), maximaal zodat max <= 10
        let mMin = a;
        let mMax = 10 - a;
        // Als er geen geldige m is (a te groot), gebruik a als m
        if (mMin > mMax) mMin = mMax = a;
        let m = floor(random(mMin, mMax + 1));
        let p = random(periodes);
        let type = random(types);
        let answer;
        if (type === 'amplitude')  answer = a;
        if (type === 'periode')    answer = p;
        if (type === 'evenwicht')  answer = m;
        return { a, m, p, type, answer };
    }

    // Genereer 10 vragen met unieke antwoorden
    // Maximaal 200 pogingen om oneindige loop te voorkomen
    let selected = [];
    let usedAnswers = new Set();
    let pogingen = 0;

    while (selected.length < 10 && pogingen < 200) {
        pogingen++;
        let q = maakGolf();
    ...
    // Fallback: als na 200 pogingen nog geen 10 unieke vragen,
    // vul aan met duplicaten (mag zelden voorkomen)
    while (selected.length < 10) {
        selected.push(maakGolf());
    }

    let answers = shuffle(selected.map(q => q.answer));

    // ── Vraagblokjes (rijen 1 en 2) ──
    let qi = 0;
    for (let row = 1; row < 3; row++) {
        for (let col = 0; col < COLS; col++) {
            ...
            blocks.push({
                ...
                gA: q.a, gM: q.m, gP: q.p, gType: q.type,
                text: '',
                answer: q.answer,
...
function setup() {   
    ...
    createNavigation();  // ← deze regel mist nu
...
function drawBlock(block) {
    ...
    // ── Hover: translate/scale zoals in de werkende histogramversie ──
    ...
    // Gebruik relatieve coördinaten als getransformeerd
    if (block.isQuestion && block.hoverProgress > 0) {
    ...
    // ── VRAAGBLOKJE: teken sinusgrafiek ──
    if (block.isQuestion) {
        let useRel = block.hoverProgress > 0;
        drawSinus(block, useRel);
        pop();
        return;
    }
...
function drawSinus(block, useRelativeCoords = false) {
    let a = block.gA;
    let m = block.gM;
    let p = block.gP;

    // Basiscoördinaten: relatief (0,0) 
    ...
    let gx = baseX + leftPad;
    let gy = baseY + topPad;
    let gw = CELL_SIZE - leftPad - rightPad;
    let gh = CELL_SIZE - topPad - botPad;
    ...
    function toX(t)   { return gx + (t / tMax) * gw; }
    function toY(val) { return gy + gh - ((val - yMin) / yRange) * gh; }
    ...
    // ── T-as tikjes en labels ──
    for (let t = tStep; t <= tMax + 0.001; t += tStep) {
        let tx = toX(t);
        stroke(0);
        strokeWeight(1);
        line(tx, gy + gh, tx, gy + gh + 3);
        noStroke();
        fill(0);
        textSize(7);
        textAlign(CENTER, TOP);
        let lbl = (t === Math.round(t)) ? t : nf(t, 1, 1);
        text(lbl, tx, gy + gh + 4);
    }
    ...
    for (let i = 0; i <= 120; i++) {
        let t   = (i / 120) * tMax;
        let val = m + a * sin(TWO_PI * t / p);
        vertex(toX(t), toY(val));
    }
    endShape();
    ...
    textSize(10);
    let label = '';
    if (block.gType === 'amplitude')  label = 'Amplitude?';
    if (block.gType === 'periode')    label = 'Periode?';
    if (block.gType === 'evenwicht')  label = 'Evenwichtsstand?';
    text(label, baseX + CELL_SIZE / 2, baseY + CELL_SIZE - 16);
    textStyle(NORMAL);
}
